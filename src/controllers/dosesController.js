const service = require("../services/dosesService");
...
module.exports = {
  ...
  async buscarPorId(req, res) {
    try {
      ...
      if (!item) return res.status(404).json({ error: "Dose não encontrada" });
      res.json(item);
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: "Erro ao buscar dose" });
    }
  },
  ...
  async atualizar(req, res) {
    try {
      ...
      if (!existe) return res.status(404).json({ error: "Dose não encontrada" });
  ...
  async deletar(req, res) {
    try {
      ...
      console.error(err);
      res.status(500).json({ error: "Erro ao deletar dose" });
    }
  }
};
